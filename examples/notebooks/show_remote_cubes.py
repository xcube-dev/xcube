...
def show_remote_cubes():
    ...
    cube_names = []
    df = pd.DataFrame(columns=['cube_name', 'chunksize (time, lat, lon)', 'var_nums', 'variables', 'start_date', 'end_date'])
    total_size = 0
    for filepath in sorted(obs_file_system.ls(path)):
        if filepath.endswith('.zarr'):
            ...
            df = df.append({'cube_name': filename, 'chunksize (time, lat, lon)': chunksize, 
                            'var_nums':len(var_list), 'variables':(str(var_list)).replace('[','').replace(']','').replace("'",""), 
