...
class GetDataStoreTest(unittest.TestCase):
    ...
    def test_get_data_store_instance_from_pool(self):
        pool = DataStorePool(
            {"dir": DataStoreConfig("file", store_params=dict(root="."))}
        )
    ...
    def test_normalize(self):
        pool = DataStorePool(
            {"@dir": DataStoreConfig("directory", store_params=dict(root="."))}
        )
...
class DataStorePoolTest(unittest.TestCase):
    ...
    def test_from_dict_no_store_params(self):
        store_configs = {"ram-1": {"store_id": "memory"}}
    ...
    def test_from_json_file_env(self):
        ...
        self._assert_file_ok(
            "json", root_1="/test_1", root_2="/test_2", use_env_vars=True
        )

    def test_from_yaml_file_env(self):
        self._assert_file_ok("yaml", use_env_vars=True)
        self._assert_file_ok(
            "yaml", root_1="/test_1", root_2="/test_2", use_env_vars=True
        )

    def _assert_file_ok(
        self, format_name: str, root_1="/root1", root_2="/root2", use_env_vars=False
    ):
        if use_env_vars:
            store_configs = self._get_test_config(
                root_1="${_TEST_ROOT_1}", root_2="${_TEST_ROOT_2}"
            )
            import os
            ...
            store_configs = self._get_test_config(root_1=root_1, root_2=root_2)
    ...
    def test_get_store(self):
        store_configs = {
            "dir-1": {"store_id": "file", "store_params": {"root": "./bibo"}},
        }
        pool = DataStorePool.from_dict(store_configs)
    ...
    def test_get_store_error(self):
        ...
        self.assertEqual(
            'Configured data store instance "dir-1" not found.', f"{cm.exception}"
        )
    ...
    def test_to_dict(self):
        ...
        self.assertEqual(
            {
                "ram": {"store_id": "memory"},
                "dir": {"store_id": "file", "store_params": {"base_dir": "bibo"}},
            },
            DataStorePool(
                {
                    "ram": DataStoreConfig(store_id="memory"),
                    "dir": DataStoreConfig(
                        store_id="file", store_params=dict(base_dir="bibo")
                    ),
                }
            ).to_dict(),
        )
    ...
    def test_multi_stores_with_params(self):
        ...
        self.assertIsInstance(pool, DataStorePool)
        self.assertEqual(
            ["local-1", "local-2", "ram-1", "ram-2"], pool.store_instance_ids
        )
    ...
    def test_get_store_instance_id(self):
        store_params_1 = {"root": "./bibo"}
        ds_config_1 = DataStoreConfig(store_id="file", store_params=store_params_1)
        ds_configs = {"dir-1": ds_config_1}
        pool = DataStorePool(ds_configs)

        store_params_2 = {"root": "./babo"}
        ds_config_2 = DataStoreConfig(store_id="file", store_params=store_params_2)
        ds_config_3 = DataStoreConfig(
            store_id="file", store_params=store_params_1, title="A third configuration"
        )

        self.assertEqual("dir-1", pool.get_store_instance_id(ds_config_1))
        self.assertEqual(
            "dir-1", pool.get_store_instance_id(ds_config_1, strict_check=True)
        )

        self.assertIsNone(pool.get_store_instance_id(ds_config_2))

        self.assertEqual("dir-1", pool.get_store_instance_id(ds_config_3))
        self.assertIsNone(pool.get_store_instance_id(ds_config_3, strict_check=True))
