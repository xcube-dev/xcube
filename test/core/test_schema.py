...
class CubeSchemaTest(unittest.TestCase):
    ...
    def test_new_chunked(self):
        ...
        schema = CubeSchema.new(cube)
        self._assert_schema(
            schema, expected_shape=cube.a.shape, expected_chunks=(1, 90, 90)
        )
    ...
    def _assert_schema(
        self, schema: CubeSchema, expected_shape=None, expected_chunks=None
    ):
    ...
    def test_constructor_with_invalid_args(self):
        ...
        self.assertEqual("shape must be a sequence of integer sizes", f"{cm.exception}")
        ...
        self.assertEqual("x_name must be given", f"{cm.exception}")

        with self.assertRaises(ValueError) as cm:
            # noinspection PyTypeChecker
            CubeSchema(schema.shape, cube.coords, y_name=None)
        self.assertEqual("y_name must be given", f"{cm.exception}")

        with self.assertRaises(ValueError) as cm:
            # noinspection PyTypeChecker
            CubeSchema(schema.shape, cube.coords, time_name=None)
        self.assertEqual("time_name must be given", f"{cm.exception}")

        with self.assertRaises(ValueError) as cm:
            CubeSchema(schema.shape[1:], schema.coords)
        self.assertEqual("shape must have at least three dimensions", f"{cm.exception}")

        with self.assertRaises(ValueError) as cm:
            CubeSchema(schema.shape, schema.coords, dims=("lat", "lon"))
        self.assertEqual("dims must have same length as shape", f"{cm.exception}")

        with self.assertRaises(ValueError) as cm:
            CubeSchema(schema.shape, schema.coords, dims=("lat", "lon", "time"))
        self.assertEqual(
            "the first dimension in dims must be 'time'", f"{cm.exception}"
        )

        with self.assertRaises(ValueError) as cm:
            CubeSchema(schema.shape, schema.coords, dims=("time", "lon", "lat"))
        self.assertEqual(
            "the last two dimensions in dims must be 'lat' and 'lon'", f"{cm.exception}"
        )

        with self.assertRaises(ValueError) as cm:
            CubeSchema(schema.shape, schema.coords, dims=schema.dims, chunks=(90, 90))
        self.assertEqual("chunks must have same length as shape", f"{cm.exception}")
        ...
        self.assertEqual(
            "missing variables 'lon', 'lat', 'time' in coords", f"{cm.exception}"
        )
        ...
        with self.assertRaises(ValueError) as cm:
            ...
            coords["lat"] = xr.DataArray(
                lat.values.reshape((1, len(lat))),
                dims=("b", lat.dims[0]),
                attrs=lat.attrs,
            )
            CubeSchema(schema.shape, coords, dims=schema.dims, chunks=(1, 90, 90))
        self.assertEqual(
            "variables 'lon', 'lat', 'time' in coords must be 1-D", f"{cm.exception}"
        )
        ...
        with self.assertRaises(ValueError) as cm:
            ...
            CubeSchema(schema.shape, coords, dims=schema.dims, chunks=(1, 90, 90))
        self.assertEqual(
            "number of labels of 'lat' in coords does not match shape",
            f"{cm.exception}",
        )
    ...
    def test_new_with_cube(self):
        ...
        with self.assertRaises(ValueError) as cm:
            CubeSchema.new(cube)
        self.assertEqual("cube is empty", f"{cm.exception}")
        ...
        with self.assertRaises(ValueError) as cm:
            CubeSchema.new(cube)
        self.assertEqual(
            "cube has no valid spatial coordinate variables", f"{cm.exception}"
        )
        ...
        with self.assertRaises(ValueError) as cm:
            CubeSchema.new(cube)
        self.assertEqual(
            "cube has no valid time coordinate variable", f"{cm.exception}"
        )
        ...
        with self.assertRaises(ValueError) as cm:
            CubeSchema.new(cube)
        self.assertEqual(
            "all variables must have same dimensions, but variable 'c' has dimensions ('q',)",
            f"{cm.exception}",
        )
        ...
        with self.assertRaises(ValueError) as cm:
            CubeSchema.new(cube)
        self.assertEqual(
            "all variables must have same chunks, but variable 'b' has chunks (1, 45, 90)",
            f"{cm.exception}",
        )
        ...
        with self.assertRaises(ValueError) as cm:
            CubeSchema.new(cube)
        self.assertEqual(
            "dimension 'lat' of variable 'a' has chunks of different sizes: (44, 43, 46, 47)",
            f"{cm.exception}",
        )
...
class GetDatasetChunksTest(unittest.TestCase):
    ...
    def test_different_chunks(self):
        ...
        self.assertEqual({"time": 1, "x": 10, "y": 25}, get_dataset_chunks(dataset))
...
class GetDatasetXYVarNamesTest(unittest.TestCase):
    def test_find_by_cf_standard_name_attr(self):
        dataset = xr.Dataset(
            coords=dict(
                u=xr.DataArray(
                    [1, 2, 3], attrs=dict(standard_name="projection_x_coordinate")
                ),
                v=xr.DataArray(
                    [1, 2, 3], attrs=dict(standard_name="projection_y_coordinate")
                ),
            )
        )
    ...
    def test_find_by_cf_long_name_attr(self):
        dataset = xr.Dataset(
            coords=dict(
                u=xr.DataArray(
                    [1, 2, 3], attrs=dict(long_name="x coordinate of projection")
                ),
                v=xr.DataArray(
                    [1, 2, 3], attrs=dict(long_name="y coordinate of projection")
                ),
            )
        )
        self.assertEqual(("u", "v"), get_dataset_xy_var_names(dataset))
        ...
        dataset = xr.Dataset(
            coords=dict(
                a=xr.DataArray([1, 2, 3], attrs=dict(long_name="longitude")),
                b=xr.DataArray([1, 2, 3], attrs=dict(long_name="latitude")),
            )
        )
    ...
    def test_find_by_var_name(self):
        ...
        self.assertEqual(("x", "y"), get_dataset_xy_var_names(dataset))
        ...
        self.assertEqual(("lon", "lat"), get_dataset_xy_var_names(dataset))
        ...
        self.assertEqual(("longitude", "latitude"), get_dataset_xy_var_names(dataset))
