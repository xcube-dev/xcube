...
class PyramidTest(unittest.TestCase):
    ...
    def create_test_dataset(cls, shape, chunks=None):
        ...
        if chunks:
            ...
            return xr.Dataset(dict(a=a, b=b)).chunk(
                chunks={dims[i]: chunks[i] for i in range(len(dims))}
            )
        else:
    ...
    def _assert_levels_ok(
        self, levels, expected_num_levels, expected_shapes, expected_chunks
    ):
