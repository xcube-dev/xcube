...
class EvaluateDatasetTest(unittest.TestCase):
    ...
    def get_test_dataset(cls):
        return xr.Dataset(
            dict(
                a=(("y", "x"), [[0.1, 0.2, 0.4, 0.1], [0.5, 0.1, 0.2, 0.3]]),
                b=(
                    ("y", "x"),
                    [[0.4, 0.3, 0.2, 0.4], [0.1, 0.2, 0.5, 0.1]],
    ...
    def test_compute_dataset_without_processed_variables(self):
        ...
        np.testing.assert_array_almost_equal(computed_dataset.c.values, np.array([1.0]))
        np.testing.assert_array_almost_equal(
    ...
    def test_compute_dataset_with_processed_variables(self):
        dataset = self.get_test_dataset()
        computed_dataset = evaluate_dataset(
            dataset,
            processed_variables=[
                ("a", None),
                ("b", dict(valid_pixel_expression=None)),
                ("c", dict(expression="a + b", load=True)),
                ("d", dict(valid_pixel_expression="c > 0.4", load=True)),
            ],
        )
        self.assertIsNot(computed_dataset, dataset)
        self.assertIn("x", computed_dataset)
        ...
        self.assertIsInstance(
            computed_dataset.c.data, np.ndarray
        )  # load=True --> load c as numpy array
        self.assertIsInstance(
            computed_dataset.d.data, np.ndarray
        )  # load=True --> load d as numpy array
