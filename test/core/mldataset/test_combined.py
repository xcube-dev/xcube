...
class CombinedMultiLevelDatasetTest(unittest.TestCase):
    def test_it(self):
        ml_ds_1 = BaseMultiLevelDataset(get_test_dataset(("noise_1", "noise_2")))
        ml_ds_2 = BaseMultiLevelDataset(get_test_dataset(("noise_3", "noise_4")))
        ml_ds_3 = BaseMultiLevelDataset(get_test_dataset(("noise_5", "noise_6")))

        ml_ds = CombinedMultiLevelDataset([ml_ds_1, ml_ds_2, ml_ds_3])
        self.assert_ml_dataset_structure_ok(ml_ds)
        ml_ds.close()

        ml_ds = CombinedMultiLevelDataset(
            [ml_ds_1, ml_ds_2, ml_ds_3], combiner_function=None
        )
        self.assert_ml_dataset_structure_ok(ml_ds)
        ml_ds.close()
