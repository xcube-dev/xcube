...
LOCAL_MODULE_DIR = os.path.join(PARENT_DIR, "test_data", "user_code")
LOCAL_MODULE_ZIP = os.path.join(PARENT_DIR, "test_data", "user_code.zip")
LOCAL_PREFIXED_MODULE_ZIP = os.path.join(
    PARENT_DIR, "test_data", "user_code_prefixed.zip"
)
...
class CodeConfigTest(unittest.TestCase):
    ...
    def test_from_github_release(self):
        ...
        self.assertEqual(
            "xcube_byoa_1.processor:process_dataset", code_config.callable_ref
        )
    ...
    def test_for_local_from_inline_code(self):
        ...
        self.assertRegex(
            os.path.basename(local_code_config.file_set.path), "xcube-byoa-*."
        )
        self.assertRegex(
            local_code_config.callable_ref, "xcube_byoa_*.:process_dataset"
        )
        # CodeConfigs from for_local() shall be able to load callable
    ...
    def test_for_local_from_prefixed_zip_file_set(self):
        ...
        self.assertEqual("processor:process_dataset", local_code_config.callable_ref)
        # CodeConfigs from for_local() shall be able to load callable
        self.assertTrue(callable(local_code_config.get_callable()))
    ...
    def test_for_local_from_github_release(self):
        ...
        self.assertEqual(
            "xcube_byoa_ex1.processor:process_dataset", local_code_config.callable_ref
        )
        # CodeConfigs from for_local() shall be able to load callable
        self.assertTrue(callable(local_code_config.get_callable()))
    ...
    def test_for_service_from_file_set_dir(self):
        ...
        self.assertTrue(service_code_config.file_set.is_local_zip())
        self.assertRegex(
            os.path.basename(service_code_config.file_set.path), "xcube-byoa-*."
        )
        self.assertEqual("processor:process_dataset", service_code_config.callable_ref)
    ...
    def test_for_service_from_github_release(self):
        ...
        self.assertEqual(
            "xcube_byoa_1.processor:process_dataset", service_code_config.callable_ref
        )
