...
class TestThreadedProgressObservers(unittest.TestCase):
    ...
    def test_api_delegate(self, m):
        ...
        progress_state = ProgressState(label="test", total_work=0.0, super_work=10.0)
        ...
        self.assertEqual(
            "Both, api_uri and access_token must be given.", str(e.exception)
        )
    ...
    def test_callback(self, m):
        ...
        m.put(
            "https://xcube-gen.test/api/v1/jobs/tomtom/iamajob/callback",
            json=expected_callback,
        )
    ...
    def test_threaded_progress_on_end(self):
        _mock_patch = patch(
            "xcube.core.gen2.progress._ThreadedProgressObserver._stop_timer"
        )
        _mock = _mock_patch.start()
