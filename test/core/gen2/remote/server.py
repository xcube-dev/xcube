...
def _get_generate_cube_result(
    ...
    if process is not None:
        ...
        raise werkzeug.exceptions.BadRequest(f"invalid job ID: {job_id}")

    output = process.stdout.readlines() if process.stdout is not None else None
        ...
        yaml.dump(
            {
                "test": {
                    ...
                    "store_id": "file",
                    "store_params": {"root": local_base_dir},
                }
            },
            stream,
        )
