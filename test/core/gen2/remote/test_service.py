...
class ServiceTest(unittest.TestCase):
    ...
    def _test_service_byoa(self, inline_code: bool):
        ...
        if inline_code:
            ...
            user_code_path = os.path.normpath(
                os.path.join(
                    PARENT_DIR, "..", "..", "byoa", "test_data", user_code_filename
                )
            )
        ...
        request_dict = {
            "input_configs": [{"store_id": "@test", "data_id": "DATASET-1.zarr"}],
            "cube_config": {},
        ...
        try:
            ...
            print("Cube info result:\n", json.dumps(result_json, indent=2), flush=True)
            self.assertEqual(
                {
                    "status": "ok",
                    "status_code": 200,
        ...
        try:
            ...
            result_json = result.to_dict()
            print(
                "Cube generator result:", json.dumps(result_json, indent=2), flush=True
            )
            self.assertEqual("ok", result_json.get("status"))
            self.assertEqual(201, result_json.get("status_code"))
            self.assertEqual({"data_id": "OUTPUT.zarr"}, result_json.get("result"))
        except CubeGeneratorError as e:
            print("Status code:\n", e.status_code, flush=True)
            print("Remote output:\n", e.remote_output, flush=True)
            print("Remote traceback:\n", e.remote_traceback, flush=True)
    ...
    def test_service_empty_cube(self):
        ...
        self.assertIn(
            "An empty cube has been generated ", result_dict.get("message", "")
        )
        self.assertIn(
            "No data has been written at all.", result_dict.get("message", "")
        )
    ...
    def test_service_invalid_request(self):
        ...
        request_dict = {
            "input_configs": [{"store_id": "@test", "data_id": "DATASET-8.zarr"}],
            "output_config": {
                "store_id": "@test",
                "data_id": "OUTPUT.zarr",
                "replace": True,
