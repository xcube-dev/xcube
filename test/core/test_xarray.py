...
class XCubeDatasetAccessorTest(unittest.TestCase):
    ...
    def test_levels(self):
        ...
        self.assertEqual(
            [(5, 180, 360), (5, 90, 180), (5, 45, 90)],
            [level.precipitation.shape for level in levels],
        )
        self.assertEqual(
            [
                ((1, 1, 1, 1, 1), (45, 45, 45, 45), (45, 45, 45, 45, 45, 45, 45, 45)),
...
class XarrayEncodingTest(unittest.TestCase):
    ...
    def assert_persistence_ok(
        ...
        a = xr.DataArray(np.array([1, np.nan, 3], dtype=np.float64), dims="x")
        a.encoding.update(_FillValue=encoded_fill_value, dtype=encoded_dtype)
        ...
        np.testing.assert_equal(ds.A.values, np.array([1, decoded_fill_value, 3]))
        ...
        np.testing.assert_equal(ds.A.values, np.array([1, encoded_fill_value, 3]))
