...
class GetCubeValuesForPointsTest(unittest.TestCase):
    ...
    def _test_get_cube_point_values(self, points):
        ...
        self.assertEqual(
            {"precipitation", "temperature", "lat_index", "lon_index", "time_index"},
            set(values.data_vars),
        )
        np.testing.assert_array_almost_equal(
            np.array(expected_prec_values, dtype=np.float64),
            values["precipitation"].values,
        )
        np.testing.assert_array_almost_equal(
            np.array(expected_temp_values, dtype=np.float64),
            values["temperature"].values,
        )
        np.testing.assert_array_equal(
            np.array(expected_time_index, dtype=np.int64), values["time_index"].values
        )
        np.testing.assert_array_equal(
            np.array(expected_lat_index, dtype=np.int64), values["lat_index"].values
        )
        np.testing.assert_array_equal(
            np.array(expected_lon_index, dtype=np.int64), values["lon_index"].values
        )

    def test_get_cube_point_indexes(self):
        cube = self._new_test_cube()

        expected_time_index = [3.0, np.nan, 7.0, 5.0, 19.0, 0.0, 4.0, 2.0]
        expected_lat_index = [
            0.0,
            650.0,
            np.nan,
        ...
        self.assertEqual(
            {"time_index", "lat_index", "lon_index"}, set(indexes.data_vars)
        )
        np.testing.assert_array_almost_equal(
            np.array(expected_time_index, dtype=np.float64),
            indexes["time_index"].values,
        )
        np.testing.assert_array_almost_equal(
            np.array(expected_lat_index, dtype=np.float64), indexes["lat_index"].values
        )
        np.testing.assert_array_almost_equal(
            np.array(expected_lon_index, dtype=np.float64), indexes["lon_index"].values
        )
...
class GetDatasetIndexesTest(unittest.TestCase):
    ...
    def _assert_get_dataset_indexes_works(self, dataset, inverse_lat=False):
        ...
        lon_coords = np.array([-190, -180.0, -179, -10.4, 0.0, 10.4, 179.0, 180.0, 190])
        expected_lon_int64 = np.array(
            [-1, 0, 1, 169, 180, 190, 359, 359, -1], dtype=np.int64
        )
        expected_lon_float64 = np.array(
            [np.nan, 0.0, 1.0, 169.6, 180.0, 190.4, 359.0, 360.0, np.nan],
            dtype=np.float64,
        )

        indexes = get_dataset_indexes(dataset, "lon", lon_coords, index_dtype=np.int32)
        ...
        lat_coords = np.array([-100, -90.0, -89, -10.4, 0.0, 10.4, 89.0, 90.0, 100])
        if not inverse_lat:
            expected_lat_int64 = np.array(
                [-1, 0, 1, 79, 90, 100, 179, 179, -1], dtype=np.int64
            )
            expected_lat_float64 = np.array(
                [np.nan, 0.0, 1.0, 79.6, 90.0, 100.4, 179.0, 180.0, np.nan],
                dtype=np.float64,
            )
        else:
            expected_lat_int64 = np.array(
                [-1, 179, 179, 100, 90, 79, 1, 0, -1], dtype=np.int64
            )
            expected_lat_float64 = np.array(
                [np.nan, 180.0, 179.0, 100.4, 90.0, 79.6, 1.0, 0.0, np.nan],
                dtype=np.float64,
            )

        indexes = get_dataset_indexes(dataset, "lat", lat_coords, index_dtype=np.int32)
        ...
        expected_time_float64 = np.array(
            [2.0, 1.96875, np.nan, 0.5, 4.0, np.nan], dtype=np.float64
        )

        indexes = get_dataset_indexes(
            dataset, "time", time_coords, index_dtype=np.int32
        )
