...
class SelectSpatialSubsetTest(unittest.TestCase):
    ...
    def test_select_spatial_subset_all_xy_bbox(self):
        ds1 = create_highroc_dataset()
        ds2 = select_spatial_subset(ds1, xy_bbox=(7.9, 53.9, 12.0, 56.4))
        self.assertIs(ds2, ds1)

    def test_select_spatial_subset_some_xy_bbox(self):
        ds1 = create_highroc_dataset()
        ds2 = select_spatial_subset(ds1, xy_bbox=(8.0, 55, 10.0, 56.0))
        self.assertEqual((3, 3), ds2.conc_chl.shape)

    def test_select_spatial_subset_none_xy_bbox(self):
        ds1 = create_highroc_dataset()
        ds2 = select_spatial_subset(ds1, xy_bbox=(13.0, 57.0, 15.0, 60.0))
        self.assertEqual(None, ds2)
    ...
    def test_select_spatial_subset_invalid_params(self):
        ...
        with self.assertRaises(ValueError) as cm:
            select_spatial_subset(
                ds1, ij_bbox=(5, 6, 7, 8), xy_bbox=(0.0, 0.0, 1.0, 2.0)
            )
        self.assertEqual(
            "Only one of ij_bbox and xy_bbox can be given", f"{cm.exception}"
        )
    ...
    def test_select_spatial_subset_descending_y_param(self):
        ...
        ds2 = select_spatial_subset(ds1, xy_bbox=(40.0, 40.0, 42.0, 42.0))
        self.assertEqual(((2,), (2,)), (ds2.lon.shape, ds2.lat.shape))

    def test_select_spatial_subset_with_gm(self):
        ds1 = new_cube(inverse_y=True)
        ds2 = select_spatial_subset(
            ds1,
            xy_bbox=(40.0, 40.0, 42.0, 42.0),
...
class SelectTemporalSubsetTest(unittest.TestCase):
    def test_invalid_dataset(self):
        ...
        self.assertEqual(
            'cannot compute temporal subset: variable "time" not found in dataset',
            f"{cm.exception}",
        )
    ...
    def test_cf_time_subset(self):
        ds1 = new_cube(
            variables=dict(analysed_sst=0.6, mask=8),
            use_cftime=True,
            time_dtype=None,
...
class SelectSubsetTest(unittest.TestCase):
    def test_all_params(self):
        ...
        np.testing.assert_almost_equal(
            np.array([10.5, 11.5, 12.5, 13.5, 14.5]), ds2.lon.values
        )

        np.testing.assert_almost_equal(
            np.array([50.5, 51.5, 52.5, 53.5, 54.5]), ds2.lat.values
        )
    ...
    def test_xy_bbox_with_large_dataset(self):
        ...
        ds_subset = select_spatial_subset(ds, xy_bbox=(0.0, 0.0, 5.0, 2.5))
        self._assert_large_dataset_subset(ds_subset)

    def test_ij_bbox_with_large_dataset(self):
        ds = self._new_large_dataset()
        ds_subset = select_spatial_subset(
            ds, ij_bbox=(129600 // 2, 64800 // 2, 129600 // 2 + 1800, 64800 // 2 + 900)
        )
        self._assert_large_dataset_subset(ds_subset)

    def _new_large_dataset(self):
        ds = new_virtual_dataset(
            lon_size=129600,
            lat_size=64800,
            time_size=5,
            lon_chunk=1296,
    ...
    def _assert_large_dataset_subset(self, ds_subset):
        ...
        self.assertEqual(
            ((1, 1, 1, 1, 1), (648, 252), (1296, 504)), ds_subset.conc_chl.chunks
        )
        ...
        ds = xr.Dataset(
            {
                "a": (
                    ["time", "y", "x"],
                    np.stack(
                        [np.full((4, 8), 1), np.full((4, 8), 2), np.full((4, 8), 3)]
                    ),
                ),
                "b": (
                    ["time", "y", "x"],
...
def new_virtual_dataset(
    ...
    lon_res = 360.0 / lon_size
    lat_res = 180.0 / lat_size

    lon_values = np.linspace(-180.0 + lon_res / 2, 180.0 - lon_res / 2, lon_size)
    lat_values = np.linspace(-90.0 + lat_res / 2, 90.0 - lat_res / 2, lat_size)
    day1 = 365 * (2021 - 1970)
    ...
    chunk_store.add_unchunked_array(
        "time",
        ["time"],
        time_values,
        attrs={"calendar": "gregorian", "units": "days since 1970-01-01"},
    )
    chunk_store.add_chunked_array(
        "conc_chl",
        ["time", "lat", "lon"],
...
class VirtualChunkStore(MutableMapping):
    ...
    def add_chunked_array(
        ...
        self._entries.update(
            self.get_array_entries_chunked(name, dims, shape, chunks, value=value)
        )
    ...
    @classmethod
    def get_array_entries_unchunked(
        cls,
        name: str,
        dims: Sequence[str],
        values: np.ndarray,
