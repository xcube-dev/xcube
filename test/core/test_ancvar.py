...
class FindAncillaryVarNamesTest(unittest.TestCase):
    ...
    def _new_array(self, dims=None, **attrs):
        ...
        for d in dims:
            s *= d[1]
        return xr.DataArray(
            np.linspace(0, 1, s).reshape([s for _, s in dims]),
            dims=([n for n, _ in dims]),
            attrs=attrs,
        )
    ...
    def test_less_strict_cf_convention(self):
        ...
        self.assertEqual(
            {
                "number_of_observations": {"a_count"},
    ...
    def test_xcube_prefix_convention(self):
        ...
        self.assertEqual(
            {"number_of_observations": {"a_count"}, "standard_error": {"a_std"}},
            find_ancillary_var_names(ds, "a"),
        )
