...
class Coords1DGridMappingTest(unittest.TestCase):
    def test_1d_j_axis_down(self):
        ...
        gm = GridMapping.from_coords(x_coords=x_coords, y_coords=y_coords, crs=GEO_CRS)
    ...
    def test_1d_anti_meridian(self):
        ...
        gm = GridMapping.from_coords(
            x_coords=xr.DataArray(lon, dims="lon"),
            y_coords=xr.DataArray(np.linspace(4.5, -4.5, 10), dims="lat"),
            crs=GEO_CRS,
        )
        self.assertEqual((8, 10), gm.size)
    ...
    def test_1d_tiles_from_coords_chunks(self):
        gm = GridMapping.from_coords(
            x_coords=xr.DataArray(da.linspace(177.5, 184.5, 8, chunks=4), dims="lon"),
    ...
    def test_1d_x_irregular(self):
        gm = GridMapping.from_coords(
            x_coords=xr.DataArray(
                [1.5, 2.5, 3.5, 4.5, 5.49, 6.5, 7.5, 8.5], dims="lon"
            ),
            y_coords=xr.DataArray(np.linspace(4.5, -4.5, 10), dims="lat"),
            crs=GEO_CRS,
        )
    ...
    def test_to_coords(self):
        ...
        gm2 = GridMapping.from_coords(
            cv["x"].astype(np.float32), cv["y"].astype(np.float32), gm.crs
        )
        cv2 = gm2.to_coords(
            xy_var_names=("a", "b"), xy_dim_names=("u", "v"), reuse_coords=True
        )
        self.assertIn("a", cv2)
        self.assertIn("b", cv2)
