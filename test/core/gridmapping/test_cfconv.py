...
class AddSpatialRefTest(S3Test, unittest.TestCase):
    ...
    def assert_add_spatial_ref_ok(self, crs_var_name, xy_dim_names):
        ...
        fs: fsspec.AbstractFileSystem = fsspec.filesystem("s3", **storage_options)
        if fs.isdir(root):
            fs.rm(root, recursive=True)
        fs.mkdirs(root, exist_ok=True)

        data_store = new_fs_data_store("s3", root=root, storage_options=storage_options)

        data_store.write_data(cube, data_id=data_id)
        cube = data_store.open_data(data_id)
        self.assertEqual({"A", "B", "time", x_name, y_name}, set(cube.variables))

        with self.assertRaises(ValueError) as cm:
            GridMapping.from_dataset(cube)
        self.assertEqual(
            ("cannot find any grid mapping in dataset",), cm.exception.args
        )
        ...
        cube = data_store.open_data(data_id)
        self.assertEqual(
            {"A", "B", "time", x_name, y_name, expected_crs_var_name},
            set(cube.variables),
        )
        self.assertEqual(expected_crs_var_name, cube.A.attrs.get("grid_mapping"))
        self.assertEqual(expected_crs_var_name, cube.B.attrs.get("grid_mapping"))
