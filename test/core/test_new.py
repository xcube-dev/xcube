...
class NewCubeTest(unittest.TestCase):
    def test_new_cube_with_bounds(self):
        ...
        self.assertEqual(-180.0, float(cube.lon_bnds[0, 0]))
        self.assertEqual(-179.0, float(cube.lon_bnds[0, 1]))
        self.assertEqual(179.0, float(cube.lon_bnds[-1, 0]))
        self.assertEqual(180.0, float(cube.lon_bnds[-1, 1]))
        self.assertEqual(-90.0, float(cube.lat_bnds[0, 0]))
        self.assertEqual(-89.0, float(cube.lat_bnds[0, 1]))
        self.assertEqual(89.0, float(cube.lat_bnds[-1, 0]))
        self.assertEqual(90.0, float(cube.lat_bnds[-1, 1]))
    ...
    def test_new_cube_with_func_vars(self):
        ...
        np.testing.assert_almost_equal(
            cube.sst[0, :, :].values, np.full((8, 16), 274.4)
        )
        np.testing.assert_almost_equal(
            cube.chl[:, 0, :].values, np.full((5, 16), 10.31)
        )
