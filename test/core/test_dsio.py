...
class ZarrDatasetIOTest(unittest.TestCase):
    def test_props(self):
        ...
        self.assertEqual(
            "Zarr file format (http://zarr.readthedocs.io)", ds_io.description
        )
        self.assertEqual({"a", "r", "w"}, ds_io.modes)
    ...
    def test_fitness(self):
        ...
        self.assertEqual(
            0.0, ds_io.fitness("http://dsio/test.zarr.zip", path_type="url")
        )
...
class GetPathOrObsStoreTest(S3Test):
    def test_path_or_store_read_from_bucket(self):
        s3_store, consolidated = get_path_or_s3_store(
            f"{MOTO_SERVER_ENDPOINT_URL}/xcube-examples/OLCI-SNS-RAW-CUBE-2.zarr",
            mode="r",
        )
    ...
    def test_path_or_store_write_to_bucket(self):
        ...
        s3_store, consolidated = get_path_or_s3_store(
            f"{MOTO_SERVER_ENDPOINT_URL}/xcube-examples/fake_cube.zarr",
            s3_client_kwargs={
    ...
    def test_path_or_store_read_from_local(self):
        path, consolidated = get_path_or_s3_store(
            "../examples/serve/demo/cube-1-250-250.zarr", mode="r"
        )
...
class ParseObsUrlAndKwargsTest(unittest.TestCase):
    ...
    def test_s3_schema(self):
        root, s3_kwargs, s3_client_kwargs = parse_s3_url_and_kwargs(
            "s3://xcube-examples/OLCI-SNS-RAW-CUBE-2.zarr",
            s3_kwargs={"anon": True},
            s3_client_kwargs={},
        )
...
class SplitBucketUrlTest(unittest.TestCase):
    def test_http(self):
        ...
        self.assertEqual("xcube-examples/OLCI-SNS-RAW-CUBE-2.zarr", root)
        endpoint_url, root = split_s3_url(
            "https://xcube-serve:9098/xcube-examples/OLCI-SNS-RAW-CUBE-2.zarr"
        )
