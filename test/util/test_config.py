...
class ToResolvedNameDictPairsTest(unittest.TestCase):
    def test_to_resolved_name_dict_pairs_and_reject(self):
        ...
        resolved = to_resolved_name_dict_pairs(
            [
                ("a*", None),
                ("b", {"name": "B"}),
...
class FlattenDictTest(unittest.TestCase):
    def test_flatten_dict(self):
        ...
        self.assertEqual(
            ...
            flatten_dict(
                {
                    "title": "DCS4COP Sentinel-3 OLCI L2C Data Cube",
                    "creator": [{"name": "BC", "url": "http://www.bc.de"}],
                }
            ),
        )
            ...
            flatten_dict(
                {
                    "title": "DCS4COP Sentinel-3 OLCI L2C Data Cube",
                    "creator": [
                        {"name": "BC", "url": "http://www.bc.de"},
                        {"name": "ACME", "url": "http://acme.com"},
                    ],
                }
            ),
        )
    ...
    def test_from_yaml(self):
        ...
        self.assertEqual("DCS4COP Sentinel-3 OLCI L2C Data Cube", d.get("title"))
        self.assertEqual(
...
class LoadConfigsTest(unittest.TestCase):
    ...
    def test_load_configs_ok(self):
        ...
        config = load_configs(
            "memory://config-1.yaml", "memory://config-2.yaml", "memory://config-3.yaml"
        )
