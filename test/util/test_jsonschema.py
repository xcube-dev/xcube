...
class JsonComplexSchemaTest(unittest.TestCase):
    def test_base_props_validated(self):
        with self.assertRaises(ValueError) as cm:
            JsonComplexSchema()
        self.assertEqual(
            "exactly one of one_of, any_of, all_of must be given", f"{cm.exception}"
        )

        with self.assertRaises(ValueError) as cm:
            JsonComplexSchema(
                one_of=[JsonStringSchema(), JsonIntegerSchema()],
    ...
    def test_to_dict(self):
        self.assertEqual(
            {
                "oneOf": [
                    {"multipleOf": 5, "type": "integer"},
                    {"multipleOf": 3, "type": "integer"},
                ]
            },
            JsonComplexSchema(
                one_of=[
                    JsonIntegerSchema(multiple_of=5),
...
class JsonSimpleSchemaTest(unittest.TestCase):
    def test_base_props_validated(self):
        ...
        self.assertEqual("factory must be callable", f"{cm.exception}")

        with self.assertRaises(ValueError) as cm:
            JsonSimpleSchema("integer", serializer="int")
        self.assertEqual("serializer must be callable", f"{cm.exception}")
...
class JsonIntegerSchemaTest(unittest.TestCase):
    ...
    def test_from_instance(self):
        self.assertEqual(
            45,
            JsonIntegerSchema(minimum=0, maximum=100, multiple_of=5).from_instance(45),
        )
...
class JsonNumberSchemaTest(unittest.TestCase):
    def test_to_instance(self):
        self.assertEqual(
            0.6,
            JsonNumberSchema(exclusive_minimum=0, exclusive_maximum=1).to_instance(0.6),
        )

    def test_from_instance(self):
        self.assertEqual(
            6, JsonNumberSchema(minimum=0, maximum=10, multiple_of=2).from_instance(6)
        )
    ...
    def test_to_dict(self):
        ...
        self.assertEqual(
            {"type": ["number", "null"]}, JsonNumberSchema(nullable=True).to_dict()
        )
        self.assertEqual(
            {"type": "number", "exclusiveMinimum": 0, "maximum": 100, "multipleOf": 10},
            JsonNumberSchema(
                exclusive_minimum=0, maximum=100, multiple_of=10
            ).to_dict(),
        )
        self.assertEqual(
            {
            ...
            JsonIntegerSchema(
                minimum=100, exclusive_maximum=200, multiple_of=20
            ).to_dict(),
        )
...
class JsonStringSchemaTest(unittest.TestCase):
    def test_to_instance(self):
        ...
        self.assertEqual(
            "pieps", JsonStringSchema(min_length=0, max_length=10).to_instance("pieps")
        )
    ...
    def test_to_dict(self):
        ...
        self.assertEqual(
            {"type": ["string", "null"]}, JsonStringSchema(nullable=True).to_dict()
        )
        self.assertEqual(
...
class JsonDateSchemaTest(unittest.TestCase):
    def test_to_instance(self):
        self.assertEqual(
            "2020-06-03",
            JsonDateSchema(min_date="2020-02-01", max_date="2020-07-05").to_instance(
                "2020-06-03"
            ),
        )
    ...
    def test_to_dict(self):
        self.assertEqual(
            ...
            JsonDateSchema(min_date="2020-02-01", max_date="2020-07-05").to_dict(),
        )
        ...
        self.assertEqual(
            ...
            JsonDateSchema(
                min_date="2020-02-01", max_date="2020-07-05", nullable=True
            ).to_dict(),
        )
...
class JsonDatetimeSchemaTest(unittest.TestCase):
    def test_to_instance(self):
        self.assertEqual(
            ...
            JsonDatetimeSchema(
                min_datetime="2020-02-01T00:00:00Z", max_datetime="2020-07-05T00:00:00Z"
            ).to_instance("2020-06-12T12:30:19Z"),
        )
    ...
    def test_new_range(self):
        ...
        self.assertEqual(
            ...
            JsonDatetimeSchema.new_range(
                min_datetime="2002-01-01T00:00:00Z",
...
class JsonArraySchemaTest(unittest.TestCase):
    ...
    def test_to_instance_tuple(self):
        self.assertEqual(
            [False, 2, "U"],
            JsonArraySchema(
                items=[JsonBooleanSchema(), JsonIntegerSchema(), JsonStringSchema()]
            ).to_instance([False, 2, "U"]),
        )

    def test_from_instance(self):
        self.assertEqual(
            [False, 2, "U"], JsonArraySchema().from_instance([False, 2, "U"])
        )
    ...
    def test_tuple_validates_as_array(self):
        self.assertTupleEqual((1, 2, 3), JsonArraySchema().to_instance((1, 2, 3)))

    def test_from_instance_array_object(self):
        value = [{"name": "Bibo", "age": 15}, {"name": "Ernie", "age": 12}]
        ...
        self.assertEqual(
            [
                {"name": "Bibo", "age": 15, "deleted": False},
...
class JsonObjectSchemaTest(unittest.TestCase):
    def test_from_json_object(self):
        ...
        self.assertEqual(
            {"name": "Bibo", "age": 12, "deleted": False},
        ...
        person_schema.factory = Person
        self.assertEqual(
            Person(name="Bibo", age=12, deleted=True),
    ...
    def test_from_json_object_object(self):
        ...
        self.assertEqual(
            Assignment(person={"name": "Bibo", "age": 15, "deleted": False}),
            schema.from_instance(value),
        )
    ...
    def test_from_json_object_array_object(self):
        ...
        schema = JsonObjectSchema(
            properties=dict(persons=JsonArraySchema(items=person_schema))
        )

        value = {"persons": [{"name": "Bibo", "age": 15}, {"name": "Ernie", "age": 12}]}
    ...
    def test_process_kwargs_subset(self):
        ...
        self.assertEqual(dict(client_id="bibo", client_secret="094529g"), cred_kwargs)
        self.assertEqual(
            dict(
                geom="POINT (12.2, 53.9)",
                spatial_res=0.5,
        ...
        ds_kwargs, kwargs = schema.process_kwargs_subset(
            kwargs, ["geom", "crs", "spatial_res", "time_range", "time_period"]
        )
        self.assertEqual(
            dict(
                crs="WGS84",
    ...
    def test_to_dict(self):
        schema = JsonObjectSchema(properties=dict(consolidated=JsonBooleanSchema()))
        self.assertEqual(
            {
                "type": "object",
