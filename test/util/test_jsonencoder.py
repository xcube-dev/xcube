...
EXPECTED_OUTPUT = {**EXPECTED_PY_OUTPUT, **EXPECTED_NP_OUTPUT}
...
class ToJsonValueTest(unittest.TestCase):
    def test_items(self):
        for k in PY_INPUT.keys():
            self.assertIs(PY_INPUT[k], to_json_value(PY_INPUT[k]))
        for k in INPUT.keys():
            self.assertEqual(EXPECTED_OUTPUT[k], to_json_value(INPUT[k]))

    def test_dict(self):
        self.assertIs(PY_INPUT, to_json_value(PY_INPUT))

        self.assertIsNot(INPUT, to_json_value(INPUT))
        self.assertEqual(EXPECTED_OUTPUT, to_json_value(INPUT))

        input_dict = dict(a=PY_INPUT, b=PY_INPUT)
        self.assertIs(input_dict, to_json_value(input_dict))

        input_dict = dict(a=INPUT, b=INPUT)
        self.assertIsNot(input_dict, to_json_value(input_dict))
        self.assertEqual(
            dict(a=EXPECTED_OUTPUT, b=EXPECTED_OUTPUT), to_json_value(input_dict)
        )
    ...
    def test_list(self):
        ...
        self.assertIs(input_list, to_json_value(input_list))

        input_list = [INPUT, INPUT]
        self.assertIsNot(input_list, to_json_value(input_list))
        self.assertEqual([EXPECTED_OUTPUT, EXPECTED_OUTPUT], to_json_value(input_list))

        input_tuple = (INPUT, INPUT)
        self.assertEqual([EXPECTED_OUTPUT, EXPECTED_OUTPUT], to_json_value(input_tuple))
    ...
    def test_numpy_arrays(self):
        ...
        array = numpy.array(
            ["2020-01-02 10:00:05", "2020-01-03 14:10:36"], dtype=np.datetime64
        )
        self.assertIsNot(array, to_json_value(array))
        self.assertEqual(
            ["2020-01-02T10:00:05Z", "2020-01-03T14:10:36Z"], to_json_value(array)
        )
