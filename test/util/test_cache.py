...
class FileCacheStoreTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        try:
            ...
            stored_value_a, size_a = self.cache_store.store_value(
                "a", bytes("abc", "utf8")
            )
            stored_value_b, size_b = self.cache_store.store_value(
                "b", bytes("def", "utf8")
            )
            stored_value_c, size_c = self.cache_store.store_value(
                "c", bytes("ghi", "utf8")
            )
            self.stored_value_a = stored_value_a
            self.stored_value_b = stored_value_b
    ...
    def test_store_value(self):
        self.assertEqual(
            self.stored_value_a, os.path.join(FileCacheStoreTest.DIR, "a.dat")
        )
        self.assertEqual(
            self.stored_value_b, os.path.join(FileCacheStoreTest.DIR, "b.dat")
        )
        self.assertEqual(
            self.stored_value_c, os.path.join(FileCacheStoreTest.DIR, "c.dat")
        )
...
class CacheTest(unittest.TestCase):
    ...
    def test_store_and_restore_and_discard(self):
        ...
        self.assertEqual(
            cache_store.trace,
            "store(k1, x);store(k2, xxx);store(k3, xx);"
        ...
        self.assertEqual(cache.size, 600)
        self.assertEqual(
            cache_store.trace, "store(k4, xxxx);discard(k1, S/x);discard(k2, S/xxx);"
        )
