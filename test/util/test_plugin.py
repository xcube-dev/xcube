...
def init_plugin(ext_registry: ExtensionRegistry):
    ...
    ext_registry.add_extension(
        component=object(), point="test.util.test_plugin", name="ext1"
    )
    ext_registry.add_extension(
        component=object(), point="test.util.test_plugin", name="ext2"
    )
    ext_registry.add_extension(
        component=object(), point="test.util.test_plugin", name="ext3"
    )
...
class PluginTest(unittest.TestCase):
    ...
    def test_load_plugins_by_entry_points(self):
        plugins = load_plugins(
            [EntryPoint("test", init_plugin)], ext_registry=self.ext_registry
        )
        self.assertEqual(
            dict(
                test=dict(
                    name="test", doc="A test plugin that registers test extensions"
                )
            ),
            plugins,
        )
        self.assertTrue(
            self.ext_registry.has_extension("test.util.test_plugin", "ext1")
        )
        self.assertTrue(
            self.ext_registry.has_extension("test.util.test_plugin", "ext2")
        )
        self.assertTrue(
            self.ext_registry.has_extension("test.util.test_plugin", "ext3")
        )
    ...
    def test_load_plugins_by_bad_entry_point(self):
        plugins = load_plugins(
            [EntryPointBad("test", init_plugin)], ext_registry=self.ext_registry
        )
    ...
    def test_load_plugins_by_bad_init_plugin(self):
        plugins = load_plugins(
            [EntryPoint("test", init_plugin_bad)], ext_registry=self.ext_registry
        )
        self.assertEqual({}, plugins)
        self.assertEqual([], self.ext_registry.find_components("test.util.test_plugin"))
    ...
    def test_load_plugins_init_plugin_not_callable(self):
        plugins = load_plugins(
            [EntryPoint("test", "init_plugin_not_callable")],
