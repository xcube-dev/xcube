...
class GridToolsTest(unittest.TestCase):
    ...
    def test_get_levels(self):
        ...
        self.assertEqual(["L", 0, 1, 2, 3, 4], [row[0] for row in results])
        self.assertEqual(["H", 3, 6, 12, 24, 48], [row[1] for row in results])
        self.assertEqual(
            [
                "R",
                Fraction(60, 1),
        ...
        self.assertEqual(
            ["R (deg)", 60.0, 30.0, 15.0, 7.5, 3.75], [row[3] for row in results]
        )
        self.assertEqual(
            ["R (m)", 6679169.45, 3339584.72, 1669792.36, 834896.18, 417448.09],
    ...
    def test_find_close_resolutions(self):
        ...
        self.assertEqual(
            ["R_D (%)", 0.012, -0.036, 0.059, -0.084], [row[0] for row in results[0:5]]
        )
        self.assertEqual(["R_NOM", 45, 5, 45, 45], [row[1] for row in results[0:5]])
        self.assertEqual(
            ["R_DEN", 16696, 1856, 16688, 16712], [row[2] for row in results[0:5]]
        )
        self.assertEqual(
            [
                "R (deg)",
        ...
        self.assertEqual(
            ["R (m)", 300.03, 299.89, 300.18, 299.75], [row[4] for row in results[0:5]]
        )
        self.assertEqual(
            ["H", 66784, 66816, 66752, 66848], [row[5] for row in results[0:5]]
        )
        self.assertEqual(
            ["H0", 2087, 261, 1043, 2089], [row[6] for row in results[0:5]]
        )
