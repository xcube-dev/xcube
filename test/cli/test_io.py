...
class IOStoreTest(CliTest):
    ...
    def test_info(self):
        ...
        self.assertEqual(0, result.exit_code)
        result = self.invoke_cli(["io", "store", "info", "file", "-POW", "root=."])
        self.assertEqual(0, result.exit_code)

        # param "root" missing
        result = self.invoke_cli(["io", "store", "info", "file", "-POW", "root=42"])
        self.assertEqual(1, result.exit_code)
        self.assertEqual(
            "Error: Failed to instantiate data store 'file'"
    ...
    def test_data(self):
        ...
        result = self.invoke_cli(
            ["io", "store", "info", "file", "-D", f"root={demo_dir}"]
        )
        self.assertEqual(0, result.exit_code)
        self.assertEqual(
            (
        ...
        result = self.invoke_cli(
            ["io", "store", "data", "file", "cube-1-250-250.zarr", f"root={demo_dir}"]
        )
        self.assertEqual(0, result.exit_code)
...
class IODumpTest(CliTest):
    ...
    def test_yaml_config_short_format(self):
        result = self.invoke_cli(
            ["io", "dump", "-c", "store-conf.yml", "--short", "--json"]
        )
        self.assertEqual(0, result.exit_code)
        self.assertTrue(os.path.exists("store-dump.json"))
        result = self.invoke_cli(
            ["io", "dump", "-c", "store-conf.yml", "--short", "--yaml"]
        )
        self.assertEqual(0, result.exit_code)
        self.assertTrue(os.path.exists("store-dump.yml"))
        result = self.invoke_cli(
            ["io", "dump", "-c", "store-conf.yml", "--short", "--csv"]
        )
        self.assertEqual(0, result.exit_code)
    ...
    def test_json_config_output(self):
        result = self.invoke_cli(
            ["io", "dump", "-c", "store-conf.json", "-o", "out.json"]
        )
        self.assertEqual(0, result.exit_code)
        self.assertTrue(os.path.exists("out.json"))
        result = self.invoke_cli(
            ["io", "dump", "-c", "store-conf.json", "-o", "out.yml"]
        )
        self.assertEqual(0, result.exit_code)
        self.assertTrue(os.path.exists("out.yml"))
        result = self.invoke_cli(
            ["io", "dump", "-c", "store-conf.json", "-o", "out.txt"]
        )
        self.assertEqual(0, result.exit_code)
    ...
    def test_json_config_type(self):
        result = self.invoke_cli(
            ["io", "dump", "-c", "store-conf.json", "-t", "dataset"]
        )
        self.assertEqual(0, result.exit_code)
        self.assertTrue(os.path.exists("store-dump.json"))
