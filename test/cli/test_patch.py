        ...
        self.write_metadata(
            {
                ...
                "metadata": {
                    ...
                    "conc_chl/.zattrs": {"title": "CHL"},
                    "conc_tsm/.zattrs": {"title": "TSM"},
                },
            }
        )
        self.write_options({})
        ...
        self.assertEqual(
            {
                ...
                "time_coverage_end": "2010-01-02T00:00:00.000000000",
...
class MetadataTest(unittest.TestCase):
    def test_parse_metadata_ok(self):
        metadata = parse_metadata(
            {
                "zarr_consolidated_format": 1,
                "metadata": {".zattrs": {"title": "Humpty-Dumpty"}},
            }
        )
        self.assertEqual({".zattrs": {"title": "Humpty-Dumpty"}}, metadata)

    def test_parse_metadata_fails(self):
        with self.assertRaises(click.ClickException) as cm:
            parse_metadata([])
        self.assertEqual("Invalid consolidated metadata format", f"{cm.exception}")

        with self.assertRaises(click.ClickException) as cm:
            parse_metadata({"zarr_consolidated_format": 3, "metadata": {}})
        self.assertEqual("Unsupported consolidated metadata version", f"{cm.exception}")

        with self.assertRaises(click.ClickException) as cm:
            parse_metadata({"zarr_consolidated_format": 1, "metadata": []})
        self.assertEqual("Invalid metadata format", f"{cm.exception}")
        ...
        with self.assertRaises(click.ClickException) as cm:
            parse_metadata({"zarr_consolidated_format": 1, "metadata": {}})
        self.assertEqual("No metadata provided", f"{cm.exception}")
