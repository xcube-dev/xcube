...
class ExtractCliTest(CliDataTest):
    ...
    def _do_test_points(self, filename):
        self._do_test_points_with_options(filename, options=[], expected_length=6)
        self._do_test_points_with_options(
            filename, options=["--coords"], expected_length=6
        )
        self._do_test_points_with_options(
            filename, options=["--bounds"], expected_length=6
        )
        self._do_test_points_with_options(
            filename, options=["--refs"], expected_length=6
        )
        self._do_test_points_with_options(
            filename, options=["--indexes"], expected_length=6
        )
        self._do_test_points_with_options(
            filename,
            options=["--coords", "--bounds", "--refs", "--indexes"],
            expected_length=6,
        )

    def _do_test_points_with_options(self, filename, options=None, expected_length=0):
        base_dir = os.path.dirname(__file__)
        args = ["extract"]
        if options:
            args += options
        args += [
            os.path.join(base_dir, "..", "..", "examples", "serve", "demo", "cube.nc"),
