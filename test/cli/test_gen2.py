...
class Gen2CliTest(CliTest):
    ...
    def test_copy_zarr_gen(self):
        request_file = os.path.join(
            os.path.dirname(__file__), "gen2-requests", "copy-zarr.yml"
        )
        result = self.invoke_cli(["gen2", "-o", result_file, request_file])
        ...
        self.assertIn("Cube generated successfully after ", result_json.get("message"))
    ...
    def test_copy_zarr_info(self):
        ...
        result = self.invoke_cli(["gen2", "--info", "-o", result_file, request_file])
        self.assertIsNotNone(result)
        result_json = self.read_result_json()
    ...
    def test_copy_levels_gen(self):
        request_file = os.path.join(
            os.path.dirname(__file__), "gen2-requests", "copy-levels.yml"
        )
        result = self.invoke_cli(["gen2", "-o", result_file, request_file])
        print(result.output)
        self.assertIsNotNone(result)
    ...
    def test_internal_error(self):
        request_file = os.path.join(
            os.path.dirname(__file__), "gen2-requests", "internal-error.yml"
        )
        result = self.invoke_cli(["gen2", "-o", result_file, request_file])
        print(result.output)
        self.assertIsNotNone(result)
