...
class S3CompatibilityTest(unittest.TestCase):
    def test_s3_datasets_bucket(self):
        store = new_data_store(
            ...
            storage_options=dict(
                anon=True, client_kwargs=dict(endpoint_url=SERVER_ENDPOINT_URL)
            ),
        )
    ...
    def test_open_dataset_from_xcube_server_rel_path(self):
        ds = open_dataset(
            ...
            format_name="zarr",
            s3_kwargs={"anon": True},
            s3_client_kwargs=dict(endpoint_url=SERVER_ENDPOINT_URL),
        )
        self.assertCubeOk(ds)
    ...
    def test_open_dataset_from_xcube_server_abs_path_raises(self):
        with pytest.raises(
            ...
            open_dataset(
                "http://localhost:8080/s3/datasets/local.zarr",
                format_name="zarr",
                s3_kwargs={"anon": True},
            )
    ...
    def assertCubeOk(self, ds):
        ...
        self.assertAlmostEqual(
            0.00005656, float(np.nanmin(conc_chl_values)), delta=1e-6
        )
        self.assertAlmostEqual(
            22.4421215, float(np.nanmax(conc_chl_values)), delta=1e-6
        )
