...
class AuthMixinIdTokenTest(unittest.TestCase):
    ...
    def test_ok(self):
        ...
        auth_mixin.request = RequestMock(headers={'Authorization': f'Bearer {access_token}'})
    ...
    def test_missing_auth_config(self):
        ...
        auth_mixin.request = RequestMock(headers={'Authorization': f'Bearer my_t0k3n'})
    ...
    def test_expired_access_token(self):
        ...
        expired_token = "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCIsImtpZCI6Ik5VSkdSVUV5UWpNeE16UTVRVGMzTnpRM05URkNRa1ky" \
                        "TURGRE5qQkJSak5ETlRBeVFrWXdOUSJ9.eyJpc3MiOiJodHRwczovL3hjdWJlLWRldi5ldS5hdXRoMC5jb20vIiw" \
                        "ic3ViIjoidzJOb0RFcnlJcE5SdEFRVkgxVG9VNlhUdXJFNTQ5RldAY2xpZW50cyIsImF1ZCI6Imh0dHBzOi8veGN" \
                        "1YmUtZGV2L2FwaS8iLCJpYXQiOjE1Nzk0NTUxMDMsImV4cCI6MTU3OTU0MTUwMywiYXpwIjoidzJOb0RFcnlJcE5" \
                        "SdEFRVkgxVG9VNlhUdXJFNTQ5RlciLCJndHkiOiJjbGllbnQtY3JlZGVudGlhbHMiLCJwZXJtaXNzaW9ucyI6W11" \
                        "9.NtGuVp0yY8foR_eFiEmH0eXvCx85cvb5b1cPWCKs6LCNniUJJ3VnX5Fek08puW7Jidy-tj1UTzyG569dSHGDu3" \
                        "10Mf7xpQ9gyfZCWcaohERxsv9MrxHziqfGVnxv051rOB_c-fYyymrFnlsIqWeahcS7znvPoMovPO7E8MxVaIqxd_" \
...
class AuthMixinAccessTokenTest(unittest.TestCase):
    ...
    def test_ok(self):
        auth_mixin = AuthMixin()
        auth_mixin.request = RequestMock(headers={'Authorization': 'Bearer my_t0k3n'})
    ...
    def test_not_ok(self):
        ...
        self.assertEqual('HTTP 401: Authorization header missing (Authorization header is expected)',
                         f'{cm.exception}')

        auth_mixin.request = RequestMock(headers={'Authorization': 'Beerer my_t0k3n'})
        with self.assertRaises(ServiceAuthError) as cm:
            auth_mixin.get_access_token()
        self.assertEqual('HTTP 401: Invalid header (Authorization header must start with "Bearer")',
                         f'{cm.exception}')
        ...
        auth_mixin.request = RequestMock(headers={'Authorization': 'Bearer my t0k3n'})
        with self.assertRaises(ServiceAuthError) as cm:
            auth_mixin.get_access_token()
        self.assertEqual('HTTP 401: Invalid header (Authorization header must be Bearer token)',
                         f'{cm.exception}')
...
class AuthMixinAuthConfigTest(unittest.TestCase):
    ...
    def test_ok(self):
        ...
        self.assertEqual('xcube-dev.eu.auth0.com', auth_config.domain)
        self.assertEqual('https://xcube-dev/api/', auth_config.audience)
        self.assertEqual('https://xcube-dev.eu.auth0.com/', auth_config.issuer)
        self.assertEqual('https://xcube-dev.eu.auth0.com/.well-known/jwks.json', auth_config.well_known_jwks)
    ...
    def test_not_ok(self):
        ...
        with self.assertRaises(ServiceConfigError) as cm:
            ...
            auth_config = auth_mixin.auth_config
        self.assertEqual('HTTP 500: Missing key "Audience" in section "Authentication"',
                         f'{cm.exception}')
        ...
        with self.assertRaises(ServiceConfigError) as cm:
            # noinspection PyUnusedLocal
            auth_config = auth_mixin.auth_config
        self.assertEqual('HTTP 500: Missing key "Domain" in section "Authentication"',
                         f'{cm.exception}')
        ...
        with self.assertRaises(ServiceConfigError) as cm:
            # noinspection PyUnusedLocal
            auth_config = auth_mixin.auth_config
        self.assertEqual('HTTP 500: Value for key "Algorithms" in section "Authentication" must not be empty',
                         f'{cm.exception}')
