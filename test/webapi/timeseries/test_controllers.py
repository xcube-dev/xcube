...
class TimeSeriesControllerTest(unittest.TestCase, AlmostEqualDeepMixin):
    ...
    def test_get_time_series_with_tolerance(self):
        ...
        actual_result = get_time_series(
            ...
            "conc_tsm",
            dict(type="Point", coordinates=[2.1, 51.4]),
            start_date=np.datetime64("2017-01-16T10:10:00Z"),
            end_date=np.datetime64("2017-01-28T09:55:00Z"),
            tolerance=0,
        )
        ...
        actual_result = get_time_series(
            ...
            "conc_tsm",
            dict(type="Point", coordinates=[2.1, 51.4]),
            start_date=np.datetime64("2017-01-16T10:00:00Z"),
            end_date=np.datetime64("2017-01-28T09:55:00Z"),
    ...
    def test_get_time_series_for_point_out_of_bounds(self):
        ctx = get_timeseries_ctx()
        actual_result = get_time_series(
            ctx, "demo", "conc_tsm", dict(type="Point", coordinates=[-150.0, -30.0])
        )
        expected_result = []
    ...
    def test_get_time_series_for_polygon(self):
        ...
        actual_result = get_time_series(
            ...
            dict(
                type="Polygon",
                coordinates=[
                    [[1.0, 51.0], [2.0, 51.0], [2.0, 52.0], [1.0, 52.0], [1.0, 51.0]]
                ],
            ),
            agg_methods=["mean", "count"],
        )
...
class CollectTimeSeriesResultTest(unittest.TestCase, AlmostEqualDeepMixin):
    ...
    def test_all_types_converted_correctly(self):
        ...
        time_series_ds = xr.Dataset(
            data_vars=dict(
                time=xr.DataArray(
                    pd.date_range(start="2010-04-05", periods=4, freq="1D"), dims="time"
                ),
        ...
        result = collect_timeseries_result(
            time_series_ds, {"a": "a", "b": "b", "c": "c"}, max_valids=None
        )
    ...
    def test_num_valids(self):
        ...
        time_series_ds = xr.Dataset(
            data_vars=dict(
                time=xr.DataArray(
                    pd.date_range(start="2010-04-05", periods=5, freq="1D"), dims="time"
                ),
        ...
        result = collect_timeseries_result(
            time_series_ds, {"a": "a", "b": "b", "c": "c"}, max_valids=-1
        )
        self.assertEqual(
            [
        ...
        result = collect_timeseries_result(
            time_series_ds, {"a": "a", "b": "b", "c": "c"}, max_valids=2
        )
        self.assertEqual(
            [
                {"a": 2.0, "b": None, "c": 0.7, "time": "2010-04-08T00:00:00Z"},
    ...
    def test_count(self):
        ...
        time_series_ds = xr.Dataset(
            data_vars=dict(
                time=xr.DataArray(
                    pd.date_range(start="2010-04-05", periods=3, freq="1D"), dims="time"
                ),
        ...
        result = collect_timeseries_result(
            time_series_ds, {"count": "c"}, max_valids=-1
        )
        self.assertEqual(
            [
