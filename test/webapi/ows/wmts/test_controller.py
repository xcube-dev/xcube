...
def get_test_res_path(path: str) -> str:
    return os.path.normpath(os.path.join(os.path.dirname(__file__), "res", path))
...
class WmtsControllerTest(unittest.TestCase):
    ...
    def test_get_wmts_capabilities_xml_crs84(self):
        ...
        actual_xml = get_wmts_capabilities_xml(
            self.wmts_ctx, "http://bibo", tms_id=WMTS_CRS84_TMS_ID
        )
    ...
    def test_get_wmts_capabilities_xml_web_mercator(self):
        ...
        with open(get_test_res_path("WMTSCapabilities-OSM.xml")) as fp:
            expected_xml = fp.read()
        actual_xml = get_wmts_capabilities_xml(
            self.wmts_ctx, "http://bibo", WMTS_WEB_MERCATOR_TMS_ID
        )
        # Do not delete, useful for debugging
        # print(80 * '=')
        # print(actual_xml)
...
class WmtsControllerXmlGenTest(unittest.TestCase):
    ...
    def test_get_operations_metadata(self):
        element = get_operations_metadata_element(
            self.wmts_ctx, "https://bibo", WMTS_WEB_MERCATOR_TMS_ID
        )
        self.assertEqual(
    ...
    def test_get_tile_matrix_set_crs84_element(self):
        tiling_scheme = TilingScheme.GEOGRAPHIC.derive(min_level=0, max_level=2)
    ...
    def test_get_tile_matrix_set_web_mercator_element(self):
        tiling_scheme = TilingScheme.WEB_MERCATOR.derive(min_level=0, max_level=2)
...
class WmtsCrs84BboxTest(unittest.TestCase):
    def test_get_crs84_bbox_ok(self):
        ...
        gm = GridMapping.regular(
            (100, 100), p0, (1000.0, 1000.0), pyproj.CRS.from_string("EPSG:3035")
        )
